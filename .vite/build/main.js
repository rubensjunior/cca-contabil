...
dotenv.config({ path: require$$1.join(electron.app.getAppPath(), ".env") });
...
function createWindow() {
    ...
    async (_, data) => {
      try {
        ...
        return {
          ...
          invoiceUrl: pendingPayment?.invoiceUrl || subscription.invoiceUrl || subscription.checkoutUrl
        };
      } catch (err) {
  ...
  electron.ipcMain.handle("asaas:get-subscription-status", async (_, subscriptionId) => {
    try {
  ...
  electron.ipcMain.handle("asaas:get-invoice-url", async (_, subscriptionId) => {
    try {
      const payments = await AsaasService.getSubscriptionPayments(subscriptionId);
      const pendingPayment = payments.find((p) => p.status === "PENDING");
      const sub = await AsaasService.getSubscription(subscriptionId);
  ...
  electron.ipcMain.handle("asaas:cancel-subscription", async (_, subscriptionId) => {
    try {
  ...
  electron.ipcMain.handle("asaas:check-customer", async (_, cpfCnpj) => {
    try {
